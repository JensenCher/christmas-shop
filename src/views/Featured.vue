<template>
  <div class="flex items-center justify-center bg-[#EBF5FD] text-[#181028]">
    <div class="flex max-w-3xl flex-col items-center justify-center p-8">
      ...
      <p class="px-10 text-center text-sm">
        Discover the magic of the season with our handpicked Featured Products.
      </p>
      <p class="px-10 text-center text-sm">
        From enchanting decorations to delightful gifts, these curated items are
        sure to add touch of joy to our festive celebrations.
      </p>
      <div class="flex flex-row items-center justify-center gap-3 py-10">
        <button
          ...
          :class="{
            ...
            'flex aspect-square w-40 items-center justify-center bg-white duration-300': true,
          }"
          @click="setActiveProduct(product)"
        >
          <div
            v-if="product.new"
            class="absolute -left-1 -top-[0.1rem] bg-[#227293] px-2 py-1 text-white"
          >
            NEW
          </div>
          <div
            v-else-if="product.discount"
            class="absolute -left-1 -top-[0.1rem] bg-[#E45953] px-2 py-1 text-white"
          >
            {{ product.discount }}
          </div>
